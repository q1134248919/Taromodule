...
import { getEffect } from './dva';

// const AWLAYS_RESET_KEYS = ['visible', 'spinning', 'loading', 'confirmLoading'];
export const REMAIN_CORE_STATE = '$$reset_part_state';

const createNestedValueReducer = (parentKey, originValue, withExtra = {}) => (
  state,
  { payload }: { payload: { key: string; extra: object } },
) => {
  const { key } = payload;
...
const getDefaultModel = () => ({
  ...
  reducers: {
    ...
    showConfirmLoading: createNestedValueReducer('confirmLoading', true, true),
    hideConfirmLoading: createNestedValueReducer('confirmLoading', false, true),
...
const enhanceEffects = (effects = {}, namespace) => {
  ...
  function createExtraCall(sagaEffects, config: any = {}) {
    const { put, call } = sagaEffects;
    return function* extraCallEffect(serviceFn, args, message: any = {}) {
      let result;
      ...
      try {
        ...
        if (confirmLoading) {
          yield put({ type: 'hideConfirmLoading', payload: { key, extra: { done } } });
        }
        if (spinning) {
          yield put({ type: 'hideSpinning', payload: { key, extra: { done } } });
        }
      }
...
function extend(defaults, properties?) {
  ...
  Object.assign(model.reducers, {
     ...
     */
    resetState(state, { payload = {} }: { payload: any }) {
